...
BasisOnMeshDofsNodes(std::shared_ptr<Partition::Manager> partitionManager, PyObject *specificSettings) :
  BasisOnMeshDofsNodesStructured<Mesh::StructuredRegularFixedOfDimension<D>,BasisFunctionType>::BasisOnMeshDofsNodesStructured(partitionManager, specificSettings), physicalExtent_({0.0})
{
  this->meshWidth_ = 0;
...
BasisOnMeshDofsNodes(std::shared_ptr<Partition::Manager> partitionManager, std::array<element_no_t, D> nElements, std::array<double, D> physicalExtent) :
  BasisOnMeshDofsNodesStructured<Mesh::StructuredRegularFixedOfDimension<D>,BasisFunctionType>::BasisOnMeshDofsNodesStructured(partitionManager, nullptr), physicalExtent_(physicalExtent)
{
...
computeMeshWidth()
{
  ...
  if (D > 1 || nElements[0] != 0)
  {
    for (int coordinateIndex = 0; coordinateIndex != D; coordinateIndex++)
    {
      double meshWidthCurrentDirection = physicalExtent_[coordinateIndex] / double(nElements[coordinateIndex] * BasisOnMeshBaseDim<1,BasisFunctionType>::averageNNodesPerElement());
      ...
      else if(fabs(this->meshWidth_ - meshWidthCurrentDirection) > 1e-14)
      {
        ...
        LOG(ERROR) << "mesh width: " << this->meshWidth_ << ", other: " << physicalExtent_[coordinateIndex] << "/" << double(nElements[coordinateIndex] * BasisOnMeshBaseDim<1,BasisFunctionType>::averageNNodesPerElement())
