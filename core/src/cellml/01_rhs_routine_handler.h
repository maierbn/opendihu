...
public:
  ...
  //! which series term index n of exp_n = (1+1/n)^n is used to approximate the exponential function
...
protected:
  ...
  int approximateExponentialFunctionSeriesIndex_;         //< which series term index n of exp_n = (1+1/n)^n is used to approximate the exponential function
  ...
  void (*rhsRoutineSingleInstance_)(void *context, double t, double *states, double *rates, double *algebraics, double *parameters);  //< function pointer to the rhs routine that can compute several instances of the problem in parallel. Data is assumed to contain values for a state contiguously, e.g. (state[1], state[1], state[1], state[2], state[2], state[2], ...). The first parameter is a this pointer.
