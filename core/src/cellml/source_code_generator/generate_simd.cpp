...
generateSourceFileSimdAutovectorization(std::string outputFilename) const
{
  std::stringstream simdSource;
  simdSource << "#include <math.h>" << std::endl
    << cellMLCode_.header.str() << std::endl;
  ...
  // add footer
  simdSource << cellMLCode_.footer.str() << std::endl;
...
void CellMLSourceCodeGenerator::
generateSourceFileExplicitVectorization(std::string outputFilename) const
{
}
    ...
    while(!source.eof())
    {
      ...
      if (!headerDone)
      {
        if (line.find(" * STATES") == 0)  // line in OpenCOR generated input file of type " * STATES[55] is P_C_SR in component razumova (milliM)."
        {
          ...
          {
            LOG(FATAL) << "The CellML file \"" << sourceFilename_ << "\" contains more than " << index << " states "
        ...
        else if (line.find(" * ALGEBRAIC") == 0)  // line in OpenCOR generated input file of type " * ALGEBRAIC[35] is g_Cl in component sarco_Cl_channel (milliS_per_cm2)."
        {
          ...
          {
            LOG(FATAL) << "The CellML file \"" << sourceFilename_ << "\" contains more than " << index << " intermediates "
      ...
      {
        cellMLCode_.header << line << std::endl;
      }
      ...
      else
      {
        if (!headerDone)
          ...
          cellMLCode_.footer << line << std::endl;
