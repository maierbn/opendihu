...
containsNanOrInf()
{
  if (this->values_)
  {
    ...
    for (int i = 0; i < values.size(); i++)
    {
      ...
      {
        if (!std::isfinite(values[i][componentNo]) || fabs(values[i][componentNo]) > 1e+75)
        {
          LOG(ERROR) << "containsNanOrInf(): value " << i << "/" << values.size() << ", component " << componentNo << "/" << nComponents << ": " << values[i][componentNo];
          return true;
        }
      }
    }
