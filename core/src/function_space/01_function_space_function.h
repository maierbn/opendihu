...
public:
  ...
  virtual double phi(int dofIndex, std::array<double,MeshType::dim()> xi, element_no_t elementNoLocal) const = 0;

  //! evaluate the derivative of Phi(xi) w.r.t xi_i, where i is given by derivativeIdx, i.e. Phi_{dofIndex,derivativeIdx}(xi)
  virtual double dphi_dxi(int dofIndex, int derivativeIdx, std::array<double,MeshType::dim()> xi, element_no_t elementNoLocal) const = 0;
  ...
  std::array<double,MeshType::dim()> gradPhi(int dofIndex, std::array<double,MeshType::dim()> xi, element_no_t elementNoLocal) const;

  //! evaluate the basis function corresponding to element-local dof dofIndex at xi, xi lives in [0,1]^D
  static double phiHexahedralMesh(int dofIndex, std::array<double,MeshType::dim()> xi);

  //! evaluate the derivative of Phi(xi) w.r.t xi_i, where i is given by derivativeIdx, i.e. Phi_{dofIndex,derivativeIdx}(xi)
  static double dphi_dxiHexahedralMesh(int dofIndex, int derivativeIdx, std::array<double,MeshType::dim()> xi);
