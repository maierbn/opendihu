...
public:
  ...
  static void transfer(const std::shared_ptr<std::vector<std::shared_ptr<Data::OutputConnectorData<FunctionSpaceType1,nComponents1a,nComponents1b>>>> transferableSolutionData1,
...
class SolutionVectorMapping<
  ...
  std::vector<std::shared_ptr<Data::OutputConnectorData<FunctionSpaceType2,nComponents2a,nComponents2b>>>   // <fieldVariableType,componentNo,prefactor>
>
{
public:
  //! transfer the data from transferableSolutionData1 to transferableSolutionData2, as efficient as possible, where there are multiple slots that could be transferred (e.g. at cellmlAdapter), use the one specified by transferSlotName
  static void transfer(const std::shared_ptr<std::pair<std::vector<Vec>,std::vector<std::shared_ptr<FieldVariableType1>>>> transferableSolutionData1,
