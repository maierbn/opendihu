            ...
            OutputConnection &outputConnection)
{
  ...
  if (transferableSolutionData1->empty())
  {
    LOG(ERROR) << "Trying to transfer data from an empty entry to a single vector. Types: " << std::endl
      << StringUtility::demangle(typeid(Data::OutputConnectorData<FunctionSpaceType1,nComponents1a,nComponents1b>).name()) << std::endl
      << StringUtility::demangle(typeid(Data::OutputConnectorData<FunctionSpaceType2,nComponents2a,nComponents2b>).name());
    return;
  }
            ...
            OutputConnection &outputConnection)
{
  ...
  if (transferableSolutionData2->empty())
  {
    LOG(ERROR) << "Trying to transfer data from a single entry to an empty vector. Types: " << std::endl
      << StringUtility::demangle(typeid(Data::OutputConnectorData<FunctionSpaceType1,nComponents1a,nComponents1b>).name()) << std::endl
      << StringUtility::demangle(typeid(std::vector<std::shared_ptr<Data::OutputConnectorData<FunctionSpaceType2,nComponents2a,nComponents2b>>>).name());
    return;
  }
/*
  SolutionVectorMapping<OutputConnectorDataType1,OutputConnectorDataType2>::transfer(
  ...
  // the following is analogous to the code in output_connector_data_transfer_fibers_emg.tpp where there are two nested vectors, here it is only one
