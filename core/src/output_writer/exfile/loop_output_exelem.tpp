...
)
{
  ...
  loopOutputExelem<OutputFieldVariablesType, i+1>(fieldVariables, meshName, file);
}
             ...
             std::ofstream &file)
{
  for (auto& currentFieldVariable : currentFieldVariableVector)
  {
    ...
    if (outputExelem<typename VectorType::value_type,OutputFieldVariablesType>(currentFieldVariable, fieldVariables, meshName, file))
      return true; // break iteration
  }
  return false;  // do not break iteration 
}
