...
loopOutputNodeValues(const FieldVariablesForOutputWriterType &fieldVariables, std::string meshName,
                     std::ostream &stream, node_no_t nodeGlobalNo)
{
  // call what to do in the loop body
  if (outputNodeValues<typename std::tuple_element<i,FieldVariablesForOutputWriterType>::type>(
    std::get<i>(fieldVariables), meshName, stream, nodeGlobalNo))
    return;
  
  // advance iteration to next tuple element
  loopOutputNodeValues<FieldVariablesForOutputWriterType, i+1>(fieldVariables, meshName, stream, nodeGlobalNo);
}
...
outputNodeValues(CurrentFieldVariableType currentFieldVariable, std::string meshName,
                      std::ostream &stream, node_no_t nodeGlobalNo)
{
  VLOG(2) << "loop_output_node_values.tpp:34, outputNodeValues, field variable " << currentFieldVariable->name();
