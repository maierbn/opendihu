...
//#endif
  ...
  if (ownRankNo == 0)
  {
    ...
    for (int rankNo = 0; rankNo < this->meshPartition_->nRanks(); rankNo++)
    {
      ...
      MPIUtility::handleReturnValue(MPI_Recv(&nDofsOnRank[rankNo], 1, MPI_INT,
    ...
    for (int rankNo = 0; rankNo < this->meshPartitionPressure_->nRanks(); rankNo++)
    {
      ...
      // receive number of dofs on rank
      MPIUtility::handleReturnValue(MPI_Recv(&nPressureDofsOnRank[rankNo], 1, MPI_INT,
                                              rankNo, 0, this->meshPartition_->rankSubset()->mpiCommunicator(), MPI_STATUS_IGNORE), "MPI_Recv");
  ...
  else
  {
    ...
    MPIUtility::handleReturnValue(MPI_Send(&nDofsLocalWithoutGhosts, 1, MPI_INT,
