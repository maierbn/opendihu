...
template<typename MeshType,typename BasisFunctionType,int nComponents>
void PartitionedPetscVecNComponentsStructured<MeshType,BasisFunctionType,nComponents>::
extractComponentShared(int componentNo, std::shared_ptr<PartitionedPetscVec<FunctionSpace::FunctionSpace<MeshType,BasisFunctionType>,1>> extractedPartitionedPetscVecNComponentsStructured)
{
  ...
  {
    setRepresentationLocal();
    extractedPartitionedPetscVecNComponentsStructured->setRepresentationLocal();

    // store the original vectorLocal and vectorGlobal of the field variable to be extracted
    savedVectorLocal_ = extractedPartitionedPetscVecNComponentsStructured->valuesLocal();
    savedVectorGlobal_ = extractedPartitionedPetscVecNComponentsStructured->valuesGlobal();

    // set the local and global vector of the foreign field variable
    extractedPartitionedPetscVecNComponentsStructured->valuesLocal() = vectorLocal_[0];
    extractedPartitionedPetscVecNComponentsStructured->valuesGlobal() = vectorGlobal_[0];
  }
  else
  {
    ...
    // This is because after VecPlaceArray the extractedPartitionedPetscVecNComponentsStructured uses the space immediately after the local values to store its ghost values.
    ...
    extractedPartitionedPetscVecNComponentsStructured->setRepresentationGlobal();

    LOG(DEBUG) << "\"" << extractedPartitionedPetscVecNComponentsStructured->name() << "\": place array";
    //extractedPartitionedPetscVecNComponentsStructured->currentRepresentation_ = Partition::values_representation_t::representationGlobal;
    ierr = VecPlaceArray(extractedPartitionedPetscVecNComponentsStructured->valuesGlobal(0), extractedData_ + componentNo*this->meshPartition_->nDofsLocalWithoutGhosts()); CHKERRV(ierr);
  }
...
template<int nComponents2>
void PartitionedPetscVecNComponentsStructured<MeshType,BasisFunctionType,nComponents>::
restoreExtractedComponent(std::shared_ptr<PartitionedPetscVec<FunctionSpace::FunctionSpace<MeshType,BasisFunctionType>,nComponents2>> extractedPartitionedPetscVecNComponentsStructured)
{
  ...
  {
    // set the local and global vector of the foreign field variable
    extractedPartitionedPetscVecNComponentsStructured->valuesLocal() = savedVectorLocal_;
    extractedPartitionedPetscVecNComponentsStructured->valuesGlobal() = savedVectorGlobal_;
  ...
  else
  {
    ...
    LOG(DEBUG) << "\"" << extractedPartitionedPetscVecNComponentsStructured->name() << "\": reset array";

    // restore the data array in the extracted vector
    ierr = VecResetArray(extractedPartitionedPetscVecNComponentsStructured->getValuesContiguous()); CHKERRV(ierr);
...
template<typename MeshType,typename BasisFunctionType,int nComponents>
void PartitionedPetscVecNComponentsStructured<MeshType,BasisFunctionType,nComponents>::
extractComponentCopy(int componentNo, std::shared_ptr<PartitionedPetscVec<FunctionSpace::FunctionSpace<MeshType,BasisFunctionType>,1>> extractedPartitionedPetscVecNComponentsStructured)
{
  if (this->currentRepresentation_ == Partition::values_representation_t::representationGlobal)
  {
  ...
  extractedPartitionedPetscVecNComponentsStructured->setRepresentationLocal();
  double *valuesTarget;
  ierr = VecGetArray(extractedPartitionedPetscVecNComponentsStructured->valuesLocal(0), &valuesTarget); CHKERRABORT(this->meshPartition_->mpiCommunicator(),ierr);

  VLOG(1) << "  copy " << this->meshPartition_->nDofsLocalWithoutGhosts()*sizeof(double) << " bytes (\"" << this->name_ << "\" component " << componentNo
    << ") to \"" << extractedPartitionedPetscVecNComponentsStructured->name() << "\"";
  memcpy(
    valuesTarget,
  ...
  // restore memory
  ierr = VecRestoreArray(extractedPartitionedPetscVecNComponentsStructured->valuesLocal(0), &valuesTarget); CHKERRABORT(this->meshPartition_->mpiCommunicator(),ierr);
