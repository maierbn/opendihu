  ...
  for (element_no_t elementNo = 0; elementNo < mesh->nLocalElements(); elementNo++)
  {
    ...
    for (int i=0; i<nDofsPerElement; i++)
    {
      for (int j=0; j<nDofsPerElement; j++)
      {
        ...
        rightHandSide.setValue(dof[i], value, ADD_VALUES);
      }  // j
    }  // i
  }  // elementNo
  ...
  if (!this->data_.massMatrixInitialized())
  {
    ...
    for (element_no_t elementNo = 0; elementNo < mesh->nLocalElements(); elementNo++)
    {
      ...
      for (int i=0; i<nDofsPerElement; i++)
      {
        for (int j=0; j<nDofsPerElement; j++)
        {
          massMatrix->setValue(dof[i], dof[j], 0, INSERT_VALUES);
        }
      }
    }
    ...
    for (element_no_t elementNo = 0; elementNo < mesh->nLocalElements(); elementNo++)
    {
      ...
      for (int i=0; i<nDofsPerElement; i++)
      {
        for (int j=0; j<nDofsPerElement; j++)
        {
          ...
          massMatrix->setValue(dof[i], dof[j], integratedValue, ADD_VALUES);
        }  // j
      }  // i
    }  // elementNo
