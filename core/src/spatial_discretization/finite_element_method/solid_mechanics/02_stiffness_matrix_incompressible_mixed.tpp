...
setStiffnessMatrix(std::shared_ptr<PartitionedPetscMat<HighOrderFunctionSpaceType>> stiffnessMatrix)
{
  ...
  for (int elementNo = 0; elementNo < nElements; elementNo++)
  {
   // get indices of element-local dofs
    std::array<dof_no_t,nDisplacementsDofsPerElement> dofNoU = meshU->getElementDofNosLocal(elementNo);
    std::array<dof_no_t,nPressureDofsPerElement> dofNoP = meshP->getElementDofNosLocal(elementNo);
...
computeIncompressibilityConstraint(Vec &result)
{
  ...
  for (int elementNo = 0; elementNo < nElements; elementNo++)
  {
    ...
    // get indices of element-local dofs
    std::array<dof_no_t,nPressureDofsPerElement> dofNosLocal = mesh->getElementDofNosLocal(elementNo);
