 ...
 *  new_slow_TK_2014_12_08 model, i.e. 57 states
 */
template<>
class FastMonodomainSolver<
  Control::MultipleInstances<                       // fibers
    OperatorSplitting::Strang<
      Control::MultipleInstances<
        TimeSteppingScheme::Heun<                   // fiber reaction term
          CellmlAdapter<
            57,71,  // nStates,nIntermediates: 57,1 = Shorten, 4,9 = Hodgkin Huxley
    ...
    >
  >
> : public FastMonodomainSolverBase<57,71>
{
public:

  //! use constructor of base class
  using FastMonodomainSolverBase<57,71>::FastMonodomainSolverBase;

protected:

  //! compute one time step of the right hand side for a single simd vector of instances
  virtual void compute0DInstance(std::array<Vc::double_v,57> &states, double currentTime, double timeStepWidth, bool stimulate) override;

  //! set the initial values for all states
  virtual void initializeStates(std::array<Vc::double_v,57> &states) override;
};
