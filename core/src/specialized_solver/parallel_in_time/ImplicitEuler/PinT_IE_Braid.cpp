            ...
            braid_StepStatus status)
{
   ...
   assert(size == solution->nDofsGlobal());
  ...
  // {
   ...
   VecCopy(u->values, solution->valuesGlobal());
   ...
   // Debug Options
   LOG(DEBUG) << "--------------------------------------------------------------";
   LOG(DEBUG) << "level: " << level << ", solver: " << solver << ", size: " << size << ", t: [" << tstart << "," << tstop << "], before implicit euler:" << *solution;
   ...
   // double    *_a;
   // int sizei;
   // VecGetSize(u->values, &sizei);
   // VecGetArray(u->values,&_a);
   // for (i = 0; i < sizei; i++) {std::cout << "\n" << _a[i];}
   ...
   //for (iterator=0; iterator<u->size; iterator++) {std::cout<<u->values[iterator] << "/n";}
   deltaT = tstop - tstart;
   deltaX = (app->xstop - app->xstart) / (size - 1.0);

   LOG(DEBUG) << "solved for t: [" << tstart << "," << tstop << "], deltaT: " << deltaT << ", deltaX: " << deltaX << ", size: " << size << " solution: " << *solution;
        ...
        braid_Vector *u_ptr)
{
   ...
   // //ierr = VecGetValues(solution->valuesGlobal(), u->size, solution->functionSpace()->meshPartition()->dofNosLocal().data(), u->values); CHKERRQ(ierr);
   //int * size, *lsize;
   //VecGetSize(solution->valuesGlobal(), size);
   //VecGetLocalSize(solution->valuesGlobal(), lsize);
   //LOG(DEBUG) << &size;
   //LOG(DEBUG) << &lsize;
   //VecGetSize(u->values, size);
   //VecGetLocalSize(u->values, lsize);
   //LOG(DEBUG) << &size;
   //LOG(DEBUG) << &lsize;
