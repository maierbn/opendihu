...
void HeunAdaptiv<DiscretizableInTime>::advanceTimeSpan()
{
  ...
  //LOG(INFO) << "currentTime: " << currentTime << ", timeStepWidth: " << this->timeStepWidth_;
  ...
  if (timeStepAdaptOption_.compare("regular") == 0){
      ...
      for(double time = 0.0; time < timeSpan;)
      {
        ...
        while(true){
            ...
            //LOG(INFO) << "tolerance: " << tolerance_ << ", estimator: " << estimator_ << ", alpha: " << alpha_;
            ...
            // error is above tolerance, reject
            } else {
                // adjust timeStepWidth
                this->timeStepWidth_ = 0.9 * alpha_ * this->timeStepWidth_;
  ...
  // modified option was chosen
  } else if (timeStepAdaptOption_.compare("modified") == 0){
...
template<typename DiscretizableInTime>
double HeunAdaptiv<DiscretizableInTime>::currentHeunTime()
{
