...
'''
  ...
  def check(self, ctx):
    ...
    self.set_build_handler([
      ...
      'cd ${DEPENDENCIES_DIR}/megamol && git clone https://github.com/embree/embree.git embree; cd embree && git checkout v3.5.2 && mkdir -p build && cd build && cmake -DCMAKE_C_COMPILER='+ctx.env["cc"]+' -DCMAKE_CXX_COMPILER='+ctx.env["CC"]+' -DEMBREE_ISPC_SUPPORT=OFF -DEMBREE_TUTORIALS=OFF -DCMAKE_INSTALL_PREFIX=${PREFIX} .. && make && make install', # build embree
      'cd ${DEPENDENCIES_DIR}/megamol && git clone https://github.com/ospray/ospray.git ospray; cd ospray && git checkout v1.8.5 && mkdir -p build && cd build && cmake -DCMAKE_C_COMPILER='+ctx.env["cc"]+' -DCMAKE_CXX_COMPILER='+ctx.env["CC"]+' -DEMBREE_DIR=${PREFIX} -DCMAKE_INSTALL_PREFIX=${PREFIX} .. && make && make install', # build ospray
