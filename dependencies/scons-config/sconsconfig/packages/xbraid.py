...
class xbraid(Package):
  ...
  def __init__(self, **kwargs):
    defaults = {
        'download_url': 'https://github.com/XBraid/xbraid/archive/v2.3.0.zip',
    }
    defaults.update(kwargs)
    super(xbraid, self).__init__(**defaults)
    self.ext = '.c'
    #self.sub_dirs = [
    #    ('include/mysql', 'lib'),
    #    ('include/mysql', 'lib64'),
...
'''
    ...
    self.set_build_handler([
      "!CFLAGS='-I${PETSC_DIR}/include' make info all ",   # this may build an examples that depends on hypre which we do not have, therefore it is allowed to fail
      "mkdir -p ${PREFIX}/lib ${PREFIX}/include ",
      "ln -s ${SOURCE_DIR}/braid/*.h ${PREFIX}/include",
      "ln -s ${SOURCE_DIR}/braid/*.hpp ${PREFIX}/include",
      "ln -s ${SOURCE_DIR}/braid/libbraid.a ${PREFIX}/lib",
    ])
      
    self.libs = ["braid"]
    self.extra_libs = ['m']
    self.headers = ["braid.h"]
        
  def check(self, ctx):
    env = ctx.env
    ctx.Message('Checking for xbraid ...        ')
    self.check_options(env)

    res = super(xbraid, self).check(ctx)
