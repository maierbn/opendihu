...
\begin{equation*}
  \begin{array}{lll}
    H(\bfx,t) = \dfrac1{\sqrt{4\pi\,c\,t}}\exp\left(-\dfrac{x^2}{4\,a\,t}\right).
  \end{array}
\end{equation*}
...
\subsection{Principal invariants}
...
Strain energy density function $\Psi(\bfC) = \Psi(I_1, I_2, I_3)$ for isotropic hyperelastic materials. To compute $\bfS = 2\,\p{\Psi(\bfC)}{\bfC}$ derivatives w.r.t. to the invariants are required. (p. 216 Holzapfel Nonlinear Solid Mechanics)
\begin{equation*}
  \begin{array}{lll}
...
For the solution of the nonlinear problem the elasticity tensor $\bbC$ has to be computed (Holzapfel p.261):
\begin{equation*}
  \begin{array}{lll}
...
For incompressible and nearly incompressible analyses it is necessary to use a mixed formulation to avoid convergence problems (locking).
...
\begin{equation*}
  ...
  \end{array}
\end{equation*}
This formula for the strain which is linear in the DOFs is not possible for general 3D descriptions, as for $\eps = 1/2 (\bfF^\top \bfF-\bfI)$ we have a quadratic dependence in the displacement then. However for beam elements and 1D elements it is valid.
...
\begin{equation}\label{eq:Wint}
  \begin{array}{lll}
    \delta W_\text{int} = \delta \hat{\bfu}^\top \left(\ds\int_{\Omega} \p{\bfeps_\text{dev}}{\hat{\bfu}} : \bfS_\text{dev}(\hat{\bfu}) \,\d V - \ds\int_{\Omega} \p{\eps_\text{vol}}{\hat{\bfu}}\,p \,\d V\right)
  \end{array}
\end{equation}
or in index notation
\begin{equation*}
  \begin{array}{lll}
    \delta W_\text{int} = \delta\hat{\bfu}_i \left(\ds\int_{\Omega} \p{\bfeps_{\text{dev},jk}}{\hat{\bfu}_i} \bfS_{\text{dev},jk}(\hat{\bfu}) \,\d V - \ds\int_{\Omega} \p{\eps_\text{vol}}{\hat{\bfu}_{i}}\,p \,\d V\right)
  \end{array}
\end{equation*}
...
\begin{equation*}
  \begin{array}{lll}
    \delta\hat{\bfu}^\top \ub{
      \ds\int_{\Omega} \p{\bfeps_\text{dev}}{\hat{\bfu}} : \bfS_\text{dev}(\hat{\bfu}) \,\d V - \ds\int_{\Omega} \p{\eps_\text{vol}}{\hat{\bfu}}\,p \,\d V 
      - \ds\int_{\Omega} \bff_{B}\,\d V - \ds\int_{\Gamma_f}  \bff_{\Gamma_f}\,\d S 
    }{=:\bfR(\hat{\bfu})} = 0
  \end{array}
\end{equation*}
This nonlinear equation $\bfR(\hat{\bfu})$ has now to be solved for $\hat{\bfu}$.

The Newton-Raphson scheme linearizes the equation around a start value $\hat{\bfu}^{(k)}$.
\begin{equation*}
  \begin{array}{lll}
    \bfR(\hat{\bfu}^{(k+1)}) = \bfR\Big|_{\hat{\bfu}=\hat{\bfu}^{(k)}} + \d{\bfR(\hat{\bfu})}{\hat{\bfu}}\Big|_{\hat{\bfu}=\hat{\bfu}^{(k)}} \ub{\big(\hat{\bfu}^{(k+1)}-\hat{\bfu}^{(k)}\big)}{=:Δ\hat{\bfu}^{(k)}} + \O\big((\hat{\bfu}^{(k+1)}-\hat{\bfu}^{(k)})^2\big).
  \end{array}
\end{equation*}
The iterative procedure starts with a start value $\hat{\bfu}^{(0)} = \hat{\bfu}_0$ and repeats to compute ${\hat{\bfu}^{(k+1)} = \hat{\bfu}^{(k)} + Δ\hat{\bfu}^{(k)}}$. The increment $Δ\hat{\bfu}^{(k)}$is computed by assuming $\bfR(\hat{\bfu}^{(k+1)}) = 0$ and solving
\begin{equation*}
  \begin{array}{lll}
    \d{\bfR(\hat{\bfu})}{\hat{\bfu}}\Big|_{\hat{\bfu}=\hat{\bfu}^{(k)}} Δ\hat{\bfu}^{(k)} = -\bfR(\hat{\bfu}^{(k)}).
  \end{array}
\end{equation*}
