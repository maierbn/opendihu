...
Domain in reference and current configuration (see \cref{fig:uniaxial_extension}):
\begin{equation*}
  \begin{array}{lll}
  ...
  \end{array}
\end{equation*}
A load $t(\bfx) = t_\text{max}/l_z^2$ is defined on the right surface area. The geometric quantities can be computed to
\begin{equation*}
  \begin{array}{lll}
...
\begin{equation*}
  \begin{array}{lll}
    \bar{\bfS} = 2\,(c_1 + c_2\,I_1)\,\bfI - 2\,c_2\bar{\bfC} = \diag\Big(2\,c_1 + 4\,c_2/\lambda,\quad -2\,c_2/\lambda, \quad -2\,c_2/\lambda\Big)
  \end{array}
\end{equation*}
...
\begin{equation*}
  \begin{array}{lll}
    \bfC : \bar{\bfS} &= 2\,c_1\,\lambda^2 \\[4mm]
    \bfS_\text{iso} &= J^{-2/3}\,\mathbb{P} : \bar{\bfS} = \bar{\bfS} - \dfrac13 (\bfC^{-1} \otimes \bfC):\bar{\bfS} \\[4mm]
     &= \bar{\bfS} - \dfrac13 \matt{
    1/\lambda^2 \cdot 2\,c_1\,\lambda^2 & 0 & 0 \\[2mm]
    0 & \lambda \cdot 2\,c_1\,\lambda^2 & 0 \\[2mm]
    0 & 0 & \lambda \cdot 2\,c_1\,\lambda^2} \\[8mm]
    &= \matt{2c_1 & 0 & 0\\[2mm] 0 & 0 & 0 \\[2mm] 0 & 0 & 0} - \dfrac13 \matt{
    2\,c_1 & 0 & 0 \\[2mm]
    0 & 2\,c_1\,\lambda^3 & 0 \\[2mm]
    0 & 0 & 2\,c_1\,\lambda^3} = \matt{
    \frac43\,c_1 & 0 & 0 \\[2mm]
    0 & -\frac23\,c_1\,\lambda^3 & 0 \\[2mm]
    0 & 0 & -\frac23\,c_1\,\lambda^3}\\[12mm]
    \bfS &= \bfS_\text{iso} + \bfS_\text{vol} = \bfS_\text{iso} + J\,p\,\bfC^{-1} = \matt{
    \frac43\,c_1 + p/\lambda^2 & 0 & 0 \\[2mm]
    0 & -\frac23\,c_1\,\lambda^3 + p\,\lambda & 0 \\[2mm]
    0 & 0 & -\frac23\,c_1\,\lambda^3 + p\,\lambda}
  \end{array}
\end{equation*}
From equilibrium we get
\begin{equation*}
  \begin{array}{lll}
    S_{22} = 0 \quad \Rightarrow \quad -\frac23\,c_1\,\lambda^3 + p\,\lambda = 0 \quad \Rightarrow \quad  p = \frac23\,c_1\,\lambda^2\\[4mm]
    S_{11} = -t_\text{max} \quad \Rightarrow \quad \frac43\,c_1 + p/\lambda^2 = -t_\text{max} \quad \Rightarrow \quad \lambda = \sqrt{\dfrac{p}{-t_\text{max} - \frac43\,c_1}}  \\[4mm]
  \end{array}
\end{equation*}
which is obviously wrong.

Different approach (p.224). Assuming Mooney-Rivlin with $c_2 = 0$ (Neo-Hookean),
\begin{equation*}
  \begin{array}{lll}
...
From equilibrium we get
\begin{equation*}
  \begin{array}{lll}S_{22} = 0 \quad \Rightarrow \quad -p\,\lambda + 2\,c_1 = 0 \quad \Rightarrow \quad p = \dfrac{2\,c_1}{\lambda}\\[4mm]
    S_{11} + t_\text{max} = 0 \quad \Rightarrow \quad -p\,\lambda^{-2} + 2\,c_1 = -2\,c_1\,\lambda^{-3} + 2\,c_1 = -t_\text{max} \quad \Rightarrow \quad \lambda = \left(1+\dfrac{t_\text{max}}{2\,c_1}\right)^3.
  \end{array}
\end{equation*}
