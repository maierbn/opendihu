...
\begin{equation*}
  ...
  \end{array}
\end{equation*}
For matrix entry $\bfK_{LaMb}$, we have
\begin{equation*}
  \begin{array}{lll}
    % L -> L
    % N -> M
    ...
    % a -> c
    % c -> d
    \bfK_{LaMb} &= \dfrac14\mathbb{C}_{cbda}\p{\phi^L(\bfx)}{x_{d}}\p{\phi^M(\bfx)}{x_{c}}
    % L -> L
    % O -> M
    % d -> a
    % a -> b
    ...
    &= \mathbb{C}_{adbc}\p{\phi^L(\bfx)}{x_{d}}\p{\phi^M(\bfx)}{x_{c}}
  \end{array}
\end{equation*}
%\begin{equation*}
%  \begin{array}{lll}
...
For the stress inside the muscle we have $\bfsigma = \bfsigma_\text{passive} + \bfsigma_\text{active}$. 
\begin{equation*}
  \begin{array}{lll}
  ...
  \end{array}
\end{equation*}
The additional term can be computed by
\begin{equation*}
  \begin{array}{lll}
...
\begin{equation*}
  ...
  \end{array}
\end{equation*}
Care must be taken when setting up the principle of virtual work, $δW_\text{int} = δW_\text{ext}$. Normally, we would have

\begin{equation*}
  \begin{array}{lll}
    δW_\text{int} = \bfI(\bfu,δ\bfu)
  \end{array}
\end{equation*}
and 
\begin{equation*}
  \begin{array}{lll}
    δW_\text{int}(\bfu,δ\bfu) = δW_\text{ext}(\bfu,δ\bfu), \quad \forall δ\bfu \in H^{1}_0(\Omega).
  \end{array}
\end{equation*}
This gives one scalar equation for each basis vector of the discretized space.  
Each equation can be thought of as equating the work that is needed externally and transferred to elastic energy internally, if one particular node is displaced along one coordinate axis by an infinitesimal amount.
This would lead to strains around the particular node. The strain itself produce a stress response. This stress together with the strain, again, produces the work.

In this formulation, the active stress term, $(\bfsigma^\text{active} : δ\bfeps)$, is part of the internal virtual work. However, this makes no sense as it is not the strain that causes the stress in this case, but the stress acts independent of the strain. This active stress actively adds virtual work to the system and should therefore be considered as external virtual work.

The quantity $\bfsigma^\text{active}$ in the equation can be seen as an additive part of the stress that results not from the material response to the strain, but from an external active stress, $\bfsigma^\text{active,applied}$, which is independently magically applied everywhere in the muscle continuum.
Therefore we have to set
\begin{equation*}
  \begin{array}{lll}
    \bfsigma^\text{active} = -\bfsigma^\text{active,applied}
  \end{array}
\end{equation*}

This leads to the following equation which has no minus signs.
\begin{equation*}
  \begin{array}{lll}
    \ds\int_{Ω} \mathbb{C}:\bfeps : δ\bfeps \, \d \bfx = \ds\int_{Ω}\bfsigma^\text{active,applied} : δ\bfeps \,\d\bfx.
  \end{array}
\end{equation*}

Furthermore, there will only be external virtual work by the active stress when the strain is negative (which means compression/contraction). Therefore, we have to use
\begin{equation*}
  \begin{array}{lll}
    \ds\int_{Ω} \mathbb{C}:\bfeps : δ\bfeps \, \d \bfx = \ds\int_{Ω}\bfsigma^\text{active,applied} : \left(δ\bfeps\right)_{-} \,\d\bfx,
  \end{array}
\end{equation*}
with
\begin{equation*}
...
(Not completely sure about the argumentation, at least this gives reasonable numerical results, while other signs and omitting the $(\cdot)_{-}$ fails.)
...
The strain-energy function for compressible material can be given in either coupled or decoupled form.
The decoupled form uses the modified strain invariants $\bar{I}_1 = J^{-2/3}\,I_1, \bar{I}_2 = J^{-4/3}\,I_2, \bar{I}_3 = 1$. It decouples the strain energy function to a volumetric elastic response, $Ψ_\text{vol}(J)$ and the isochoric elastic response $Ψ_\text{iso}(\bar{\bfC})$.
\begin{equation*}
  \begin{array}{lll}
    \Psi = \Psi_\text{vol}(J) + \Psi_\text{iso}(\bar{I}_1,\bar{I}_2)
  \end{array}
\end{equation*}
The volumetric part is often given by
\begin{equation*}
  \begin{array}{lll}
    \Psi_{vol}(J) = \kappa\,G(J) \quad\text{with, e.g.,}\\[4mm]
    \quad G(J) = \beta^{-2}(\beta\,\log(J) + J^{-\beta} - 1), \qquad \text{or}\\[4mm]
    \quad G(J) = \frac14(J^2 - 1 - 2\,\log(J)) 
  \end{array}
\end{equation*}
(Holzapfel p.233, p.244/255 \cite{holzapfel2000nonlinear})

A coupled form uses the normal strain invariants, $I_1, I_2, J$, e.g. the coupled Mooney-Rivlin model,
\begin{equation*}
  \begin{array}{lll}
...
\subsection{Nonlinear hyperelasticity}
...
According to the principle of stationary potential energy, the system is in equilibrium if the potential energy functional is stationary.
...
For the directional derivative of the internal energy functional with respect to $δ\bfu$ we get
...
We discretize the displacements, $u_a$, pressure, $p$, increments $Δu_a, Δp$ and the virtual displacements and virtual pressure, $δu_a, δp$, using two different ansatz functions for displacements and pressure, $\phi(\bfX)$ and $\psi(\bfX)$, respectively,
\begin{equation*}
  \begin{array}{lll}
...
To illustrate the algorithm, we define $(\hat{\bfu},\hat{p}) =: \bfz$ and the governing equations to be $W(\bfu,p) =: \mathcal{W}(z)=0$. We proceed by linearization:
\begin{equation*}
  \begin{array}{lll}
...
Given the number of spatial dimensions $a,b=1,\dots,D=3$, the blocks of the Jacobian or tangent stiffness matrix are given by:
\begin{equation*}
  \begin{array}{lll}
...
increments $Δ\hat\bfu^{(i)}, Δ\hat p^{(i)}$:
\begin{equation*}
  \begin{array}{lll}
...
\begin{equation*}
  \begin{array}{lll}
    ...
    \bar{\bfC} = J^{-2/3}\bfC \quad &\text{(p.228)}\\[4mm]
    \bar{\gamma}_1 = 2\left(\p{\Psi_\text{iso}(\bar{I}_1, \bar{I}_2)}{\bar{I}_1} + \bar{I}_1\,\p{\Psi_\text{iso}(\bar{I}_1, \bar{I}_2)}{\bar{I}_2}\right)\quad &\text{(p.234)}\\[4mm]
    \bar{\gamma}_2 = -2\p{\Psi_\text{iso}(\bar{I}_1, \bar{I}_2)}{\bar{I}_2},\quad\text{note, there is no $\bar{\gamma}_3$} &\text{(p.234)}\\[4mm]
...
\begin{equation*}
  \begin{array}{lll}
    ...
    \p{\bar I_5}{\bar\bfC} = \bfa_0 \otimes \bar\bfC\,\bfa_0 + \bfa_0\,\bar\bfC \otimes \bfa_0, \quad \left(\p{\bar I_5}{\bar\bfC}\right)_{ij} = a_{0,i}\,a_{0,k}\,\ub{\bar{C}_{kj}}{=\bar{C}_{jk}} + a_{0,j}\,a_{0,k}\,\ub{\bar{C}_{ki}}{=\bar{C}_{ik}} \quad &\text{(p.277)}\\[4mm]
    \dfrac{∂^2\bar{I}_5}{∂\bar{\bfC}∂\bar{\bfC}} = \p{\bar{\bfC}}(\bfa_0 \otimes \bar\bfC\,\bfa_0 + \bfa_0\,\bar\bfC \otimes \bfa_0), \\[4mm] 
     \dfrac{∂^2\bar{I}_5}{∂\bar{\bfC}_{ij}∂\bar{\bfC}_{lm}} = \dfrac12(\delta_{jm}\,a_{0,i}\,a_{0,l} + \delta_{im}\,a_{0,j}\,a_{0,l} + \delta_{jl}\,a_{0,i}\,a_{0,m} + \delta_{il}\,a_{0,j}\,a_{0,m})\\[4mm]
...
\subsubsection{Compressible Hyperelasticity}

Compressible materials in decoupled form have the same equations for stress, $\bfS$, and material tangent tensor, $\CC$, except the hydrostatic pressure, $p$, is computed from the volumetric strain energy function,
\begin{equation*}
  \begin{array}{lll}
    p = \p{Ψ_\text{vol}(J)}{J}
  \end{array}
...
The computation is analogous to the Penalty method \ref{sec:penalty} and is the same as for the incompressible case, except for computation of $p$ and $\tilde{p}$ that is used for $\mathbb{C}$.
 
For the coupled form, the algorithm is different:

\begin{equation*}
  \begin{array}{lll}
    J = \det(\bfF), \quad \bfC = \bfF^\top \bfF\\[4mm]
    \gamma_1 = 2\left(\p{\Psi(I_1, I_2, I_3)}{I_1} + I_1\,\p{\Psi(I_1, I_2, I_3)}{I_2}\right)\quad &\text{(p.248)}\\[4mm]
    \gamma_2 = -2\p{\Psi(I_1, I_2, I_3)}{I_2}\quad &\text{(p.248)}\\[4mm]
    \gamma_3 = 2\,I_3\,\p{\Psi(I_1, I_2, I_3)}{I_3}\quad &\text{(p.248)}\\[4mm]
    \bfS = 2\p{\Psi(I_1, I_2, I_3)}{\bfC} = \gamma_1\,\bfI + \gamma_2\,\bfC + \gamma_3\,\bfC^{-1} \\[4mm]
  \end{array}
\end{equation*}
\begin{equation*}
  \begin{array}{lll}
    \big(\bfC^{-1} \odot \bfC^{-1}\big)_{abcd} = \dfrac12\big(C^{-1}_{ac}\,C^{-1}_{bd} + C^{-1}_{ad}\,C^{-1}_{bc}\big) \quad &\text{(p.254)}\\[4mm]
    \delta_1 = 4\left(\dfrac{∂^2\Psi}{∂I_1\,∂I_1} + 2\,I_1\dfrac{∂^2\Psi}{∂I_1\,∂I_2} +\dfrac{∂\Psi}{∂I_2} + I_1^2\,\dfrac{∂^2\Psi}{∂I_2\,∂I_2}\right) \quad &\text{(p.261)}\\[4mm]
    \delta_2 = -4\left(\dfrac{∂^2\Psi}{∂I_1\,∂I_2} + I_1\,\dfrac{∂^2\Psi}{∂I_2\,∂I_2}\right) \quad\\[4mm]
    \delta_3 = 4\left(I_3\,\dfrac{∂^2\Psi}{∂I_1\,∂I_3} + I_1\,I_3\,\dfrac{∂^2\Psi}{∂I_2\,∂I_3}\right) \quad\\[4mm]
    \delta_4 = 4\dfrac{∂^2\Psi}{∂I_2\,∂I_2} \quad\\[4mm]
    \delta_5 = -4\,I_3\,\dfrac{∂^2\Psi}{∂I_2\,∂I_3}\\[4mm]
    \delta_6 = 4\left(I_3\,\dfrac{∂\Psi}{∂I_3} + I_3^2\,\dfrac{∂^2\Psi}{∂I_3\,∂I_3}\right)\\[4mm]
    \delta_7 = -4\,I_3\,\dfrac{∂\Psi}{∂I_3}\\[4mm]
    \delta_8 = -4\,\dfrac{∂\Psi}{∂I_2}\\[4mm]
    \mathbb{S} = (\mathbb{I} + \bar{\mathbb{I}}) / 2, \quad \mathbb{I}_{abcd} = δ_{ac}\,δ_{bd}, \quad \bar{\mathbb{I}}_{abcd} = δ_{ad}\,δ_{bc} &\text{(p.262, p.23)}\\[4mm]
    \mathbb{C} = 2\,\p{\bfS}{\bfC} = 
      \delta_1\,\bfI \otimes \bfI 
      +\delta_2\,\big(\bfI \otimes \bfC + \bfC \otimes \bfI\big)
      +\delta_3\,\big(\bfI \otimes \bfC^{-1} + \bfC^{-1} \otimes \bfI\big)
      +\delta_4\,\bfC \otimes \bfC \\[4mm] 
    \qquad\qquad+\delta_5\,\big(\bfC \otimes \bfC^{-1} + \bfC^{-1} \otimes \bfC\big)
      +\delta_6\,\big(\bfC^{-1} \otimes \bfC^{-1}\big)
      +\delta_7\,\big(\bfC^{-1} \odot \bfC^{-1}\big)
      +\delta_8\,\mathbb{S} \quad &\text{(p.261)} \\[4mm]
  \end{array}
\end{equation*}
...
In the general case with damping we use a transformation to a system of first order ODEs.
\begin{equation*}
  \begin{array}{lll}
...
\subsubsection{Dynamic nonlinear elasticity}
Governing equations
...
The linear system for each Newton iteration takes the form
\begin{equation*}
  \begin{array}{llll}
    \matt{
...
Note, the matrix is unsymmetric, it would be symmetric if $\bfl_{δ\bfu,Δ\bfv}  = \bfl_{δ\bfv,Δ\bfu}^\top$ which is the case if $\rho_0 = 1$ and $\int_{\Omega} \phi_{b}^M \phi_{a}^L \,\d V = \delta_{ab}\delta^{LM}$, i.e. lumped mass matrix with row-sum.
