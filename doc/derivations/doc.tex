...
\begin{equation*}
  \begin{array}{lll}
    \dot{\bfu}: \qquad &\dfrac{1}{dt}(\bfu^{(n+1)} - \bfu^{(n)}) = \theta \dot{\bfu}^{(n+1)} + (1-\theta) \dot{\bfu}^{(n)},\\[4mm]
    \dot{\bfv}: \qquad &\dfrac{1}{dt}(\bfv^{(n+1)} - \bfv^{(n)}) = \theta \dot{\bfv}^{(n+1)} + (1-\theta) \dot{\bfv}^{(n)},\\[4mm]
  \end{array}
\end{equation*}
...
\begin{equation}\label{eq:first_ders}
  \begin{array}{lll}
    δW_\text{int}({\bfu},{p})  = \ds\int_{\Omega}\dfrac12  S_{AB}(\bfu,p)\, \left(F_{aB}\, \phi_{(a),A}^M + F_{aA}\, \phi_{(a),B}^M\right)δ\hat{u}_{a}^M \,\d V\\[4mm]
    δW_\text{ext,dead}  = \ds\int_{\Omega} B_{(a)}^M\,\phi_{(a)}^M\, \phi_{(a)}^M\,δ\hat{u}^M_a \,\d V +\ds\int_{∂\Omega}  \bar{T}^L_{(a)}\,\phi_{(a)}^L\, \phi_{(a)}^M\,δ\hat{u}^M_a\,\d S \\[4mm]
    D_{δp}\Pi_L(\bfu) = \ds\int_\Omega \big(J(\bfu) - 1)\,δp\,\d V 
  \end{array}
\end{equation}
...
\quad    -\bfR_{δ\bfu, (L,a)}^{\text{ext,old}} =  \dfrac1{dt}\ds\int_{\Omega} \rho_0\,\hat{v}^{(n+1),M}_a\,\phi_{(a)}^M \phi_{(a)}^L \,\d V + \ds\int_{∂\Omega}  \bar{T}_a^M \,\phi_{(a)}^M\, \phi_{(a)}^L\,\d S \\[4mm]
\quad    -\bfR_{δ\bfu, (L,a)}^{\text{ext,new}} =  -\dfrac1{dt}\ds\int_{\Omega} \rho_0\,\hat{v}^{(n,M)}_a\,\phi_{(a)}^M \phi_{(a)}^L \,\d V + \ds\int_{∂\Omega}  \bar{T}_a^M \,\phi_{(a)}^M\, \phi_{(a)}^L\,\d S \\[4mm]
\quad    -\bfR_{δ\bfu, (L,a)}^{\text{ext,dead}} = -\ds\int_{\Omega} B_a^M\,\phi_{(a)}^M \phi_{(a)}^L \,\d V - \ds\int_{∂\Omega}  \bar{T}_a^M \,\phi_{(a)}^M\, \phi_{(a)}^L\,\d S \\[4mm]
    ...
    d \mbox{ blocks of subvectors with length } u_{\text{dofs}}\\[4mm]
    -\bfR_{δp,L} = \ds\int_\Omega (J(\bfu^{(n+1)}(\bfX)-1)\,\psi^L\,\d V \\[4mm]
