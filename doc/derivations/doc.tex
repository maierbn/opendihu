...
\begin{equation*}
  ...
  \end{array}
\end{equation*}
After each update of $\bfu$, we enforce incompressibility by solving
\begin{equation*}
  \begin{array}{lll}
    \bfK\,\bfu^{(n+1,\text{new})} &= \bff - \bfC\,\bfv^{(n+1)} - \bfM\,\bfa(\bfu^{(n+1)},\bfv^{(n+1)},t^{(n+1)})\\[4mm]
     &= \bfK\,\bfu^{(n+1)}
  \end{array}
\end{equation*}
Note that in the program, the vector used for $\bfu$ contains the values of displacements and pressure.

The problem is this algorithm does not work, because the incompressibility is not enforced correctly on both $u$ and $v$.
