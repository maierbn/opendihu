...
The jacobian determinant, $J$, is compute from discretized quantities as follows.
\begin{equation*}
  \begin{array}{lll}
...
\begin{equation*}
  \begin{array}{lll}
    D_{δ\bfu}\Pi_\text{ext} = -δW_\text{ext} &= -\ds\int_{\Omega} (B_a - \rho_0\,\ddot{u}_a) δu_a\,\d V - \ds\int_{∂\Omega}  \bar{T}_a δu_a\,\d S\\[4mm]
     &= -\ds\int_{\Omega} (B_a - \rho_0\,\ddot{u}_a) δ\hat{u}^M_a\,\phi_{(a)}^M \,\d V - \ds\int_{∂\Omega}  \bar{T}_a δ\hat{u}^M_a\,\phi_{(a)}^M\,\d S
  \end{array}
\end{equation*}
...
\begin{equation}\label{eq:first_ders}
  \begin{array}{lll}
    ...
    δW_\text{ext}  = \ds\int_{\Omega} (B_a - \rho_0\,\ddot{u}_a) \phi_{(a)}^M\,δ\hat{u}^M_a \,\d V +\ds\int_{∂\Omega}  \bar{T}_a\, \phi_{(a)}^M\,δ\hat{u}^M_a\,\d S \\[4mm]
...
\begin{equation*}
  \begin{array}{lll}
    \bfR_{δ\bfu, (L,a)} = -D_{δ\bfu, (L,a)}\Pi_L(\bfu,p) = -δW_{\text{int}, (L,a)} + δW_{\text{ext}, (L,a)} = \bfR_{δ\bfu, (L,a)}^{\text{int}} + \bfR^{\text{ext}}_{δ\bfu, (L,a)}  \mbox{ with}\\[4mm]
\quad    \bfR_{δ\bfu, (L,a)}^{\text{int}} = - \ds\int_{\Omega}\dfrac12  S_{AB}\, \left(F_{aB}\phi_{(a),A}^L + F_{aA}\phi_{(a),B}^L\,\right) \,\d V \\[4mm]
\quad    \bfR_{δ\bfu, (L,a)}^{\text{ext}} = \ds\int_{\Omega} (B_a - \rho_0\,\hat{\ddot{u}}_a) \phi_{(a)}^L \,\d V + \ds\int_{∂\Omega}  \bar{T}_a \phi_{(a)}^L\,\d S \\[4mm]
    d \mbox{ blocks of subvectors with length } u_{\text{dofs}}\\[4mm]
    \bfR_{δp,L} = -D_{δp, L}\Pi_L(\bfu,p) = \ds\int_\Omega \big(1-J)\,\psi^L\,\d V \\[4mm]
