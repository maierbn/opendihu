...
\begin{equation*}
  \begin{array}{lll}
    ...
    \bfk_{δp,Δ\bfu} & \bfzero} \, \matt{Δ\hat{\bfu} \\[2mm] Δ\hat{p}} =  \matt{δW_{\text{ext}} \\[2mm] \bfzero},
  \end{array}
\end{equation*}
where the blocks of the Jacobian or tangent stiffness matrix are given by
\begin{equation*}
  \begin{array}{lll}
    \bfk_{δ\bfu,Δ\bfu,(L,a),(M,b)} = \ds\int_\Omega \p{\phi_{(a)}^L}{X_B}\tilde{k}_{abBD}\p{\phi_{(b)}^M}{X_D}\,\d V \qquad \text{with}\qquad 
