...
\begin{equation*}
  \begin{array}{lll}
    \ub{\ds\int_{\Omega} \bfP : ∇_\bfX \delta\bfu\,\d V}{=δW_\text{int}} = \ub{\ds\int_{\Omega} (\bfB - \rho_0\,\ddot{\bfu})\,\d V + \ds\int_{∂\Omega}  \bar{\bfT}\cdot δ\bfu\,\d s}{=δW_\text{ext}}
  \end{array}
\end{equation*}
...
\begin{equation*}
  ...
  \end{array}
\end{equation*}
The displacement field, $\bfu$, is discretized using ansatz functions with coefficient vector $\hat{\bfu}$, yielding
\begin{equation}\label{eq:pvw_r}
  \begin{array}{lll}
    \ub{δW_\text{int}(\hat{\bfu}) - δW_\text{ext}}{=:\bfR} = 0.
  \end{array}
\end{equation}
...
\begin{equation}\label{eq:directional_derivative_w_int}
  \begin{array}{lll}
    D_{Δ\hat{\bfu}^{(k)}}δW_\text{int}(\hat{\bfu},δ\hat{\bfu}) = \ds\int_\Omega \p{\delta u_a}{X_B}(\ub{δ_{ab}\,S_{AB} + F_{aA}\,F_{bC}\,C_{ABCD}}{=\tilde{k}_{abBD}})\p{Δu_b}{X_D}\,\d V
  \end{array}
\end{equation}
with
\begin{equation*}
...
\begin{equation*}
  \begin{array}{lll}
    k_{ij} = \ds\int_\Omega \p{\phi_a}{X_B}\tilde{k}_{abBD}\p{\phi_b}{X_D}\,\d V
  \end{array}
\end{equation*}
with appropriate mappings $i \mapsto (a,B)$ and $j \mapsto (b,D)$.
...
\begin{equation*}
  ...
  \end{array}
\end{equation*}
See Holzapfel \cite{holzapfel2000nonlinear}
$\C_\text{vol}$ p.254,
$\C_\text{iso}$ p.255,
$\bar{\C}$ p.262,
$\bfS_\text{iso}$ p.234
...
\begin{equation*}
  \begin{array}{lll}
    \bfS_\text{iso} = J^{-2/3}\mathbb{P}:\bar{\bfS}, \quad \bar{S} = 2\p{\Psi_\text{iso}(\bar{I_1},\bar{I_2})}{\bar{\bfC}} = \bar{\gamma_1}\,\bfI + \bar{\gamma_2}\,\bar{\bfC}.
  \end{array}
\end{equation*}
