...
\begin{equation}\label{eq:directional_derivative_w_int}
  \begin{array}{lll}
    D_{Δ\hat{\bfu}^{(k)}}δW_\text{int}(\hat{\bfu},δ\hat{\bfu}) = \ds\int_\Omega \p{\delta u_a}{X_B}(\ub{δ_{ab}\,S_{AB} + F_{aA}\,F_{bC}\,C_{ABCD}}{=k_{abBD}})\p{Δu_b}{X_D}\,\d V
  \end{array}
\end{equation}
with
\begin{equation*}
  \begin{array}{llll}
    ...
    k_{abBD}, \quad &\dots & \text{entries of stiffness matrix.}\\[4mm]
  \end{array}
\end{equation*}
