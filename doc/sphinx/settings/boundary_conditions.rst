...
In the example in :numref:`dof_numbering_parallel`, a mesh with :math:`7 \times 5` nodes is partitioned to two processes. The local dof numbers are given in blue. Process 0 has ghost dofs shown by the red numbers. If, e.g., the value of the thirt-to-last dof in the top row should be prescribed, you have to set both dof 25 on rank 0 and dof 12 on rank 1 to the same prescribed value as both subdomains are adjacent to this node.
