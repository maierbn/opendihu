...
for solver in ["lu", "cg","gmres", "gamg"]:
  ...
  for elem in [200,500,1480,5000]:
    df_plot=df_solver[df_solver['~nElements']== elem]
    means = df_plot.groupby(['nRanks']).mean()
    errors = df_plot.groupby(['nRanks']).std()
    #print(means)
    
    means.plot(ax=ax,y="durationSolve_implicitSolver",logx=False, logy=True, linestyle=linestyle_list[solver_cnt],color=next(color), marker=next(marker), yerr=errors)
    legend_list.append(solver+"-"+str(elem))
