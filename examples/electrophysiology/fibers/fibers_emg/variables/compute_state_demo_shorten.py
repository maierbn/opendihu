# example to demonstrate the compute state of the subcellular points: 
# not yet activated fibers are not computed and on activated fibers, only points not in equilibrium are computed
# run as:
# mpirun -n 4 ./fast_fibers_shorten_emg ../settings_fibers_emg.py compute_state_demo_shorten.py

import os

# scenario name for log file
scenario_name = "compute_state_demo_shorten"
...
motor_units = [
  {"radius": 40.00, "activation_start_time": 0.00, "stimulation_frequency": 0.01, "jitter": [0.1*random.uniform(-1,1) for i in range(100)]},    # low number of fibers
  {"radius": 42.35, "activation_start_time": 0.01, "stimulation_frequency": 20, "jitter": [0.1*random.uniform(-1,1) for i in range(100)]},
  {"radius": 42.35, "activation_start_time": 0.01, "stimulation_frequency": 20, "jitter": [0.1*random.uniform(-1,1) for i in range(100)]},
  {"radius": 42.35, "activation_start_time": 0.01, "stimulation_frequency": 20, "jitter": [0.1*random.uniform(-1,1) for i in range(100)]},
  {"radius": 42.35, "activation_start_time": 0.01, "stimulation_frequency": 20, "jitter": [0.1*random.uniform(-1,1) for i in range(100)]},
  {"radius": 42.35, "activation_start_time": 0.01, "stimulation_frequency": 20, "jitter": [0.1*random.uniform(-1,1) for i in range(100)]},
  {"radius": 42.35, "activation_start_time": 0.01, "stimulation_frequency": 20, "jitter": [0.1*random.uniform(-1,1) for i in range(100)]},
  {"radius": 45.00, "activation_start_time": 0.032, "stimulation_frequency": 25, "jitter": [0.1*random.uniform(-1,1) for i in range(100)]},
  {"radius": 48.00, "activation_start_time": 1, "stimulation_frequency": 22.46, "jitter": [0.1*random.uniform(-1,1) for i in range(100)]},
  {"radius": 51.42, "activation_start_time": 1, "stimulation_frequency": 20.28, "jitter": [0.1*random.uniform(-1,1) for i in range(100)]},
]

# timing parameters
# -----------------
end_time = 500.0                      # [ms] end time of the simulation
...
output_timestep = 10             # [ms] timestep for output surface EMG, 0.5
