# This example mimics the settings of the all_active.py scenario for multidomain_with_fat
# It is used to compare the runtimes of fibers_fat_emg with multidomain_with_fat for the same computatio# It is used to compare the runtimes of fibers_fat_emg with multidomain_with_fat for the same computation.
# multidomain: 105 min for 24ms
# this example: 6 s for 24ms
#
# Comment in the code for 37x37 fibers instead of 13x13 fibers.
...
for mu_no in range(n_motor_units):
  ...
  motor_units.append(
  {
    ...
    "activation_start_time": 1e-3*random.random()*20,              # [s] when to start activating this motor unit, here it is a ramp
...
end_time = 24.0                      # [ms] end time of the simulation
...
fiber_distribution_file = input_directory+"/MU_fibre_distribution_13x13_25.txt"
#fiber_distribution_file = input_directory+"/MU_fibre_distribution_37x37_25.txt"

# stride for sampling the 3D elements from the fiber data
# a higher number leads to less 3D elements
sampling_stride_x = 1  # 3 for 37x37 yields the same mesh as 1 for 13x13
sampling_stride_y = 1  # 3 for 37x37 yields the same mesh as 1 for 13x13
...
if True:
  ...
  # only on rank 0
  if (int)(sys.argv[-2]) == 0:
    for mu_no,item in enumerate(motor_units[0::5]):
      print("MU {}".format(mu_no*5))
