...
n_motor_units = 500   # number of motor units
...
for mu_no in range(n_motor_units):
  ...
  motor_units.append(
  {
    ...
    "activation_start_time": 0.01*mu_no,              # [s] when to start activating this motor unit, here it is a ramp
...
fiber_file              = input_directory+"/input/left_biceps_brachii_109x109fibers.bin"
...
fiber_distribution_file = input_directory+"/input/MU_fibre_distribution_109x109_500.txt"
...
if True:
  ...
  # only on rank 0
  if (int)(sys.argv[-2]) == 0:
    for mu_no,item in enumerate(motor_units[0::100]):
      print("MU {}".format(mu_no*100))
