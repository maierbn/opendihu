# Electrophysiology with Shorten model as the rhs. 
#
...
dt_1D = 1e-1                     # timestep width of diffusion
dt_splitting = max(dt_0D,dt_1D)  # overall timestep width of splitting

output_timestep = 1e1            # timestep for output files
...
if "hodgkin_huxley-razumova" in cellml_file:
  ...
  mappings = {
    ...
    ("outputConnectorSlot", 1): "Razumova/activation",      # expose activation .
    ("outputConnectorSlot", 2): "Razumova/activestress",
   }
...
def handle_result(n_instances, time_step_no, current_time, states, algebraics, name_information, additional_argument):
  ...
  # output some values 
  print("t: {}, Vm: {}, gamma: {}, activation: {}".format(current_time, Vm, active_stress, activation))
