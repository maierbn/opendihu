...
#    ],
    ...
    "3Dmesh": [
       ...
       {"name": "3Dmesh_elasticity_quadratic+3DFatMesh_elasticity_quadratic",  "xiTolerance": 0.5, "enableWarnings": False, "compositeUseOnlyInitializedMappings": True, "fixUnmappedDofs": True, "defaultValue": 0},    # mapping uses mappings of submeshes (i.e. 3Dmesh_elasticity_quadratic->3Dmesh)
    ],
    "3DFatMesh":  [
  ...
  "Coupling": {
    ...
    "durationLogKey":         "duration_total_with_prestretch",
    "timeStepOutputInterval": 1,
    "endTime":                variables.end_time,
    "connectedSlotsTerm1To2": None, #{0:5},       # data transfer is configured using global option "connectedSlots"
    "connectedSlotsTerm2To1": None,       # transfer nothing back
    
    # prestretch computation
    "Term1": {
      "HyperelasticitySolver": {
        ...
        "loadFactors":                list(np.logspace(-1,0,2)),        # load factors
      ...
      "Coupling": {
        ...
        "Term2": {        # solid mechanics
          "MuscleContractionSolver": {
            ...
            "lambdaDotScalingFactor":       1,                         # scaling factor for the output of the lambda dot slot, i.e. the contraction velocity. Use this to scale the unit-less quantity to, e.g., micrometers per millisecond for the subcellular model.
            ...
            "DynamicHyperelasticitySolver": {
              ...
              "displacementsScalingFactor": 1.0,                       # scaling factor for displacements, only set to sth. other than 1 only to increase visual appearance for very small displacements
              "residualNormLogFilename":    "out/"+variables.scenario_name+"/log_residual_norm.txt",   # log file where residual norm values of the nonlinear solver will be written
              "useAnalyticJacobian":        True,                      # whether to use the analytically computed jacobian matrix in the nonlinear solver (fast)
              "useNumericJacobian":         False,                     # whether to use the numerically computed jacobian matrix in the nonlinear solver (slow), only works with non-nested matrices, if both numeric and analytic are enable, it uses the analytic for the preconditioner and the numeric as normal jacobian
