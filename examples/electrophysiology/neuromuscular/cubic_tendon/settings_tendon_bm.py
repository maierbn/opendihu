...
if True:
	i = 0
	for j in range(ny):
		variables.elasticity_dirichlet_bc[k*nx*ny + j*nx + i][0] = 0.0

	j = 0
	for i in range(nx):
		variables.elasticity_dirichlet_bc[k*nx*ny + j*nx + i][1] = 0.0

initial_displacements = [[0.0,0.0,0.0] for _ in range(nx * ny * nz)]

if False: #make True to initialize with data in file
...
k = 0
variables.elasticity_neumann_bc = [{"element": k*mx*my + j*mx + i, "constantVector": [0,0,-0.5], "face": "2-", "isInReferenceConfiguration": True} for j in range(my) for i in range(mx)]
...
print(f"neumann: {variables.elasticity_neumann_bc}")
print(f"dirichlet: {variables.elasticity_dirichlet_bc}")
...
config = {
  ...
  "DynamicHyperelasticitySolver": {
    ...
    # nonlinear solver
    "relativeTolerance":          1e-100,                         # 1e-10 relative tolerance of the linear solver
    "absoluteTolerance":          1e-15,                        # 1e-10 absolute tolerance of the residual of the linear solver       
    ...
    "snesRelativeTolerance":      1e-100,                         # relative tolerance of the nonlinear solver
    "snesLineSearchType":         "l2",                         # type of linesearch, possible values: "bt" "nleqerr" "basic" "l2" "cp" "ncglinear"
    "snesAbsoluteTolerance":      1e-100,                         # absolute tolerance of the nonlinear solver
