...
with open('data/muscle_case6.txt', 'r') as f:
    lines = f.readlines()
    t6 = [float(line.split()[0]) for line in lines]
    z6= [float(line.split()[1]) for line in lines]
...
plt.figure(2)
plt.plot(t5,z5,'o', markersize=2,label="case 5")
plt.plot(t5,z5,'o', markersize=2,label="case 6")

plt.title("Comparing different reversed setups")
plt.xlabel("time (ms)")
