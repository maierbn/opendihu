...
# for j in range(my):
#   for i in range(mx):
#     variables.elasticity_neumann_bc.append({"element": k*mx*my + j*mx + i, "constantVector": [0,0, 0], "face": "2+", "isInReferenceConfiguration": True})

def update_neumann_bc(t):
  force = 0.0009
  factor = (t-3.7)/(5.0-3.7)
  if factor < 0: factor = 0
  variables.elasticity_neumann_bc = [{
    "element": j*mx + i, 
    "constantVector": [0,0, -force*factor], 		
    "face": "2-",
  ...
  # print(variables.elasticity_neumann_bc)
  config = {
    "inputMeshIsGlobal": True,
