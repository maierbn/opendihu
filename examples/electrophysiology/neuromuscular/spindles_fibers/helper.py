...
result = create_partitioned_meshes_for_settings(
    ...
    fiber_set_rank_nos=False, have_fibers=True, include_global_node_positions=include_global_node_positions)
...
if hasattr(variables, "main_bottom_traction"):
  ...
  for j in range(m2y):
    for i in range(m2x):
      variables.main_elasticity_dirichlet_bc[offset + (m2z-1)*m2x*m2y + j*m2x + i] = [None,None,0.0,None,None,None]
