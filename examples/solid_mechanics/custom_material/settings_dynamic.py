# Hyperelastic custom material, dynamic problem
import numpy as np
import sys, os
...
# Boundary conditions
# Dirichlet boundary conditions for displacements and velocities
dirichlet_bc = {}
...
for j in range(0,my):
  for i in range(0,mx):
    dirichlet_bc[j*(mx) + i] = [None,None,zpos,None,None,None]    # [u_1, u_2, u_3, v_1, v_2, v_3], None means unconstraint

if False:
  # vertical plane at x=0
  for k in range(0,mz):
    for j in range(0,my):
      dirichlet_bc[k*mx*my + j*mx + 0] = [xpos,None,None,None,None,None]

  # vertical plane at y=0
  for k in range(0,mz):
    for i in range(0,mx):
      dirichlet_bc[k*mx*my + i] = [None,ypos,None,None,None,None]

  # vertical edge at x=0, y=0
  for k in range(0,mz):
    dirichlet_bc[k*mx*my] = [xpos,ypos,None,None,None,None]

if True:
  # edge z=0, y=0
  for i in range(0,mx):
    dirichlet_bc[i] = [None,ypos,zpos,None,None,None]
...
# bottom corner
dirichlet_bc[0] = [xpos,ypos,zpos,None,None,None]
dirichlet_bc[1] = [None,ypos,zpos,None,None,None]

# Neumann boundary conditions (traction)
# constant forces at right plane (2+ = positive z direction)
# [N]
neumann_bc = [{"element": (nz-1)*nx*ny + j*nx + i, "constantVector": [0,10,10], "face": "2+"} for j in range(ny) for i in range(nx)]

#dirichlet_bc = {}
#neumann_bc = []

# material parameters
c1 = 50        # [N/cm^2] first Mooney-Rivlin parameter
c2 = 75      # [N/cm^2] second Mooney-Rivlin parameter
c =  100       # [-]      bulk modulus (high = less compressible)
material_parameters = [c1, c2, c]
rho = 10        # [1e-4 kg/cm^3] density of the material (density of water)

# load
#constant_body_force = (0,0,-9.81e-4)   # [cm/ms^2], gravity constant for the body force in negative z direction
constant_body_force = (-9.81e-4,0,0)    # gravity in negative x direction

dt = 1                # [ms] time step width
end_time = 30         # [ms] end time of simulation
output_timestep = dt  # [ms] time interval in which output files are written, set to dt to output every timestep
...
config = {
  ...
  "DynamicHyperelasticitySolver": {
    ...
    "density":                    rho,                       # density of the material
    ...
    "OutputWriter" : [
      ...
      # Paraview files
      {"format": "Paraview", "outputInterval": int(1./dt*output_timestep), "filename": "out/"+scenario_name+"/u", "binary": True, "fixedFormat": False, "onlyNodalValues":True, "combineFiles":True, "fileNumbering": "incremental"},
    ...
    "pressure": {   # output files for pressure function space (linear elements), contains pressure values, as well as displacements and velocities
      "OutputWriter" : [
        {"format": "Paraview", "outputInterval": int(1./dt*output_timestep), "filename": "out/"+scenario_name+"/p", "binary": True, "fixedFormat": False, "onlyNodalValues":True, "combineFiles":True, "fileNumbering": "incremental"},
      ]
    },
    # 3. additional output writer that writes virtual work terms
    "dynamic": {    # output of the dynamic solver, has additional virtual work values 
      "OutputWriter" : [   # output files for displacements function space (quadratic elements)
        {"format": "Paraview", "outputInterval": int(1./dt*output_timestep), "filename": "out/"+scenario_name+"/virtual_work", "binary": True, "fixedFormat": False, "onlyNodalValues":True, "combineFiles":True, "fileNumbering": "incremental"},
      ],
    },
