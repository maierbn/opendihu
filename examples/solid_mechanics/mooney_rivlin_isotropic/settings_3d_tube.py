...
neumann_bc = [{"element": (nz-1)*nx*ny + j*nx + i, "constantVector": [0,1e-2,2e-2], "face": "2+"} for j in range(ny) for i in range(nx)]
...
config = {
  ...
  "HyperelasticitySolver": {
    ...
    "materialParameters":         [5.0,10],
    "displacementsScalingFactor": 1.0,   # scaling factor for displacements
    "constantBodyForce":          [5e-3, 0.0, 0.0],   # body force in whole body region
    ...
    "snesRebuildJacobianFrequency": 1,  # frequency with which the jacobian is newly computed
