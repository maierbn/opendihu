...
if dirichlet_bc_mode == "fix_all":
  
  # fix z value on the whole x-y-plane
  for j in range(my):
    for i in range(mx):
      elasticity_dirichlet_bc[k*mx*my + j*mx + i] = [0.0,0.0,0.0,None,None,None]
...
def handle_result_hyperelasticity(result):
  ...
  if data["timeStepNo"] == 1:
    ...
    print("field variables for hyperelasticity:")
