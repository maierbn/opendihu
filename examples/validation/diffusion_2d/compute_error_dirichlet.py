...
filenames = [filename for filename in glob.glob("build_release/out_dirichlet/*.py")]
...
def analytic_solution(x, y, t):
    result, abserr = scipy.integrate.dblquad(lambda yp,xp: G_D(x,y,xp,yp,t) * u_initial(xp,yp), 0, size_x, 0, size_y)
    return result 
...
plt.grid()
plt.xlabel("t [s]")
plt.savefig("error_dirichlet.png")
plt.show()
