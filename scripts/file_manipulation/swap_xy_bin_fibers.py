...
# -*- coding: utf-8 -*-
#
# This script changes the order of the nodes by swapping x and y coordinates. This changes the coordinate system from a right-handed to a left-handed system and vice versa.
# For .bin files.
#
# usage: swap_xy_bin_fibers.py <input filename> [<output filename>]
...
output_filename = "{}.swapped".format(input_filename)
if len(sys.argv) >= 3:
...
with open(input_filename, "rb") as infile:
  ...
  print("swap x and y numbering in file with {} x {} fibers\n".format(n_fibers_x,n_fibers_y))
  ...
  with open(output_filename,"wb") as outfile:
    ...
    for y in range(n_fibers_y):
      for x in range(n_fibers_x):
        fiber = streamlines[x*n_fibers_y + y]
        # loop over points of fiber
        for point_no in range(n_points_whole_fiber):
